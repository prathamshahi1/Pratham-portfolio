...
async function main() {
  ...
  // If high-quality pre-rendered resume exists (e.g. built on local dev with Chrome and checked into git), preserve it on CI!
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 50000) {
    copyToOutputs();
    console.log('✓ High-quality pre-rendered LaTeX resume preserved at ' + outputPath);
    return;
  }

  await buildWithPDFKitAsync();
}
